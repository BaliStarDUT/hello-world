                ...
                if (deltaSize > 0) {
                    ...
                    if (cache.readerIndex() >= deltaSize) { // 可以清理
                        ...
                        if (oldCapacity > DEFAULT_MAX_BUFFER_SIZE) { // 尝试收缩
                            ...
                            if (newCapacity < (oldCapacity >> 1)) { // 至少收缩二分之一
                                try {
                                    ...
                                    logger.info(String.format("shrink cache capacity: {} - {} = {} bytes",
                                        oldCapacity,
                                        oldCapacity - newCapacity,
                                        newCapacity));
                                } catch (OutOfMemoryError ignore) {
                                    maxDirectBuffer = oldCapacity; // 未来不再超过当前容量，记录日志后继续
                                    logger.info(String.format("cache OutOfMemoryError: {} bytes", newCapacity, ignore));
                                }
                            }
                        }
                    } else { // 尝试扩容
                        ...
                        if (oldCapacity < maxDirectBuffer) {
                            ...
                            try {
                                cache.capacity(newCapacity);
                                logger.info(String.format("expand cache capacity: {} + {} = {} bytes",
                                        oldCapacity,
                                ...
                                if (maxDirectBuffer > maxDirectMemory) {
                                    ...
                                    newCapacity = maxDirectBuffer;
                                    logger.info(String.format("resize maxDirectBuffer: {} bytes", maxDirectBuffer, e));
                                    try {
                                        cache.capacity(newCapacity);
                                        logger.info(String.format("expand cache capacity: {} + {} = {} bytes",
                                                oldCapacity,
                                    ...
                                    maxDirectBuffer = oldCapacity; // 未来不再超过当前容量，记录日志后继续
                                    logger.info(String.format("cache OutOfDirectMemoryError: {} bytes", newCapacity, e));
                                }
                            } catch (OutOfMemoryError ignore) {
                                maxDirectBuffer = oldCapacity; // 未来不再超过当前容量，记录日志后继续
                                logger.info(String.format("cache OutOfMemoryError: {} bytes", newCapacity, ignore));
